...
function add_new_var(
  var_id = get_var_name(),
  selected_option = wl_manifest[0]
) {
...
function update_message(e) {
  ...
  if (null_var) {
    update_status_message("empty field", status_bad_color);
  }

  if (bad_var) {
    update_status_message("invalid variable", status_bad_color);
  }

  if (no_vars) {
    update_status_message("no variables", status_info_color);
  }
...
function show_hide_help_panel() {
  show_hide_panel(
    document.getElementById("help-panel-close-button"),
...
function import_gen_text() {
  if (document.getElementById("ie-text").value !== "") {
    ...
    document.getElementById("ie-text").innerHTML = show_hide_ie_panel();
    update_ui();
  } else {
...
function export_gen_text() {
  ...
  json_export = {
    [json_var_key]: varnames,
    [json_wl_key]: wls,
    [json_gen_key]: gen_text,
  };
...
function show_hide_ie_panel() {
  show_hide_panel(
    document.getElementById("ie-panel-close-button"),
    document.getElementById("ie-panel")
  );
  document
    .getElementById("ie-panel-import-button")
    .addEventListener("click", import_gen_text);
  document
    .getElementById("ie-panel-export-button")
    .addEventListener("click", export_gen_text);
}
...
async function setup_gui() {
  ...
  var help_panel_close_button = document.getElementById(
  ...
  document
    .getElementById("ie-panel-load-button")
    .addEventListener("click", async function () {
      ...
      document.getElementById("ie-text").value = JSON.stringify(
        example_json,
        null,
        2
      );
